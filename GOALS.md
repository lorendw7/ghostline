...
| **Stickers / emoji packs** | P1 | Start with **app-bundled sticker sets** — sending a sticker only transmits a small sticker ID, so it is near-zero-cost and clean under E2EE (no media encrypt/upload). **User-uploaded custom stickers are P2/later** (those need client-encrypt + object-storage upload like any media). |
...
| **E2EE via per-recipient fan-out** | P0 (if built) | Photos are client-encrypted before upload to self-hosted MinIO and fanned out per recipient public key — the *same* mechanism as group chat (II-bis), so the **30-member cap and the plaintext-consistency hash apply equally**. The server only ever stores ciphertext. |
...
| **Media** (images / voice / video) | **Even shorter than chat — a 3-day hard ceiling** (II-bis); stored in self-hosted MinIO on the VPS, then purged. Users are reminded to download/save anything they want to keep. | No — client-encrypted before upload. |
