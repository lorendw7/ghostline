...
- **Everything is ephemeral**: every Chat Mode message carries an expiry; once it passes, the message is deleted on both devices and purged from the server — no "keep forever" option. **Text** has a hard ceiling of one month (per-conversation adjustable down to 1 day; 7-day default); **media (images/voice/video, Moments photos) is shorter still — a 3-day hard ceiling**, and the UI reminds users to download/save anything they want to keep. Letter Mode is the deliberate exception. See GOALS Section II-bis.
