...
10. **Media/message compression — a self-study track (you write the code). Compress BEFORE you encrypt, and don't blanket-prefer lossless.**
   ...
   - **⚠️ Cost caveat for the Preserve-quality path:** originals run 10–50MB each; combined with ≤1-month expiry + free-tier R2, heavy original-photo sharing can blow the free quota — so put a **size cap or a warning** on the Original mode, and only lift it if you pay for storage.
