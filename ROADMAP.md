...
10. **Media/message compression — a self-study track (you write the code). Compress BEFORE you encrypt, and don't blanket-prefer lossless.**
   ...
   - **Lossless vs lossy — be honest about photographs.** Lossless (PNG / WebP-lossless) is right for **text, screenshots, line-art, logos, charts** (flat regions compress well). For an actual **photograph** literal lossless is often *huge* (a 24MP frame as PNG/WebP-lossless = 20–80MB, sometimes bigger than the source), so blanket lossless wastes bandwidth/storage given the **3-day media expiry** (GOALS II-bis) + free-R2 model.
   - **Policy by SOURCE, not by format — two tracks:**
     - **In-app camera snapshot / screenshot (casual):** lossy WebP/AVIF (q≈80–90), may downscale → small. Lossless only for screenshots/graphics.
     - **Shared gallery photo / DSLR (a "Preserve quality / Original" mode — the user wants this for everyday sharing of real photography):** **(1) never downscale — full resolution preserved** (the usual quality-killer is the resize, not the codec); **(2) if the source is already JPEG/HEIC, pass the original bytes through unchanged** — re-encoding a captured JPEG to PNG only bloats it for *zero* quality gain, since the lossy step already happened at capture, so "lossless relative to the source" = keep the original file; **(3) to shrink without visible loss, use AVIF / JPEG-XL / WebP at q≈95 (visually lossless ≠ mathematically lossless, indistinguishable to the eye at a fraction of true-lossless size); (4) offer a true-lossless / "send original" toggle** for photographers who insist, at the cost of size.
   - **"Visually lossless" is usually what you actually want**, not literal lossless: it gives DSLR-grade fidelity without 50MB files. **JPEG-XL** is the photographer's dream (true-lossless mode + ~20% *lossless* recompression of existing JPEGs) but 2026 browser support is still partial — treat as optional/learn.
   - **⚠️ Cost caveat for the Preserve-quality path:** originals run 10–50MB each; even with the **3-day media expiry** + free-tier R2, heavy original-photo sharing can blow the free quota — so put a **size cap or a warning** on the Original mode, and only lift it if you pay for storage. Because media is wiped within 3 days, the UI also **reminds recipients to download/save** anything worth keeping (GOALS II-bis).
...
- Connect **self-hosted PostgreSQL** (Docker on the VPS), store a users table (username + public key, **no password hash, no phone/email**) + a messages table. (For the earliest local dev you can run Postgres locally; the point is it's your own Postgres, not a managed service.)
...
- **Backend deployment — everything self-hosted on one VPS.** The whole backend lives on a **single Xserver VPS in Tokyo** (developer lives in Japan → low latency, yen billing, JP support; far better spec/yen than Vultr Tokyo): **Node main backend + Go fan-out service + PostgreSQL + Redis + MinIO** (S3-compatible media, local disk), all run with **Docker Compose** behind a reverse proxy (Caddy / optionally **Coolify** so other projects deploy with a push). No third-party DB/Redis/blob service. Plan: **2GB/3-core** (~¥792/mo) runs ghostline alone; **6GB/4-core** (~¥1,359/mo) is the sweet spot if the box doubles as a multi-project playground. Media on local disk stays bounded because it auto-expires in 3 days (clarification 10 / GOALS II-bis).
- **⚠️ Backups are now P0, not optional.** One VPS = one point of failure; if it dies, every friend's ciphertext dies with it. Run **scheduled encrypted database dumps to a *different* location** (another host, or a cheap off-site bucket like Backblaze B2 — the one acceptable external service, holding only encrypted dumps). Never let the only backup sit on the same box. Practise a restore at least once.
- The clients (mobile/desktop binaries) cost nothing to distribute; only this backend VPS needs always-on hosting; the web/PWA static client still hosts free on Vercel/Netlify/Cloudflare Pages.
